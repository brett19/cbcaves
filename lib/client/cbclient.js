...
function decodeValue(data, flags, datatype, format) {
  if (!data) {
    return null;
  }
...
// This function exists to allow a client to be connected asynchronously
//  from the creation of the object.  This is used primarily by the
//  test_harness to inject into the couchnode tests properly.
CouchbaseClient.prototype._setup = function(options) {
  // Read the options
...
CouchbaseClient.prototype.shutdown = function() {
  this._config.close();

  for (var i in this._serverLookup) {
    if (this._serverLookup.hasOwnProperty(i)) {
      var server = this._serverLookup[i];
      server.close();
    }
  }
};
