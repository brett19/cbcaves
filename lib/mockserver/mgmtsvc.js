...
function MgmtService(parent, parentNode) {
  ...
  this.parent.on('bucketDeleted', function(bucket) {
    self._closeBucketConfigStreams(bucket);
  });
}
...
MgmtService.prototype._closeBucketConfigStreams = function(bucketName) {
  for (var i = 0; i < this.configStreams.length; ++i) {
    var stream = this.configStreams[i];
    if (stream.bucket === bucketName) {
      stream.end();
    }
  }
};
